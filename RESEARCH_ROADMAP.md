...
- **★2026-08-09（同日、続き）：動的制御の壁を越えた（投機的展開＋巻き戻し）。**
  `'error <= '41` は廃止。`'loop` の「入口が静的に真」の枝で投機を始め、`'lcheck` のフレームに
  `(Vl, RCode, TmpCtr)` を退避し、脱出条件の動的化が判明したら**入口へ巻き戻して**残余化する
  （約 40 行）。巻き戻し先が入口なのは、可逆ループが入口テストを初回に真・戻り辺で偽と表明するため
  「k 反復目から続ける」残余ループが書けないから。巻き戻し後は `DYNAMICIZE-ALL` を先に、
  次に**入口テストを再特殊化**（定数化すると戻り辺を取れない＝非可逆になる）。
  **成果**：データ依存のループが初めて work 指標で基準を満たした
  （`reverse` 0.17×、`length` 0.14×、`length2` 0.13×。静的制御の 0.91×／0.86× より良い）。
  **同一ベンチを変更前の `spec_av` で回すと 3 被験とも `'41`** で落ちることを確認済み。
  ゲート：テスト 278 件緑、`[comp2](('S.swap)) == B : true`（fp2 維持）。
  **限界**：`ri_fp3` 経由の残余は正しいが巨大（`reverse` 46003 ノード、17.17×）。
  「越えた」＝停止して正しい、であって Jones 最適ではない。
  **§10.4 の前提は誤りだった**：`INV-SPEC-STEP-AV` による逆向き実行は不可能（`spec_av` は
  `X ^= X` でゴミを捨てるので単射でない）。「仕事を捨てる＝逆に走る」を本当に書くなら
  土台を `spec_av_rev` にする必要がある（別プロジェクト）。詳細は §10.6。
  ~~(ii) 代数的性質（合成則・cogen の不動点性＝第4射影の退化）~~ → **2026-08-09 に形式化＋実機測定**
  （新規 7 モジュール・`--safe`・postulate ゼロ・`check.sh` 114→121 PASS）。**主結果は
  「第 4 射影の退化はゴミの置き場所で割れる」**：dead 枝埋込（`spec_av_rev`）ではそのまま成立、
  出力の対に持つ設計では不成立で正しい形は `snd (run cogen rspec) ≡ cogen`（反証は対の非巡回性だけから）。
  → dead 枝埋込は実装上の工夫ではなく**射影の代数を保存する設計**だった。
  実機でも `[comp3](('S.spec_av))` と `[comp3_rev](('S.spec_av_rev))` が**バイト一致**することを
  新規テスト 2 件で確認（20 秒・24 秒）。合成則は `spec-compose`・`cogen-curry`・
  `tower-collapse : ∀ n → tower n ≡ cogen` まで。**なお「cogen の不動点性」という表現は
  演算子を書かないと偽**（`⟦cogen⟧(cogen) ≡ cogen` は反証済み）、
  **残り**：
  (iii) steps 側の 1.4〜5.0× を縮める＝残余のムーブを減らす作業は①と同じ土俵、
  (iv) **`ri_fp3` 経由の残余の肥大**＝束縛時刻を改善したインタプリタが要る（巻き戻しとは別問題）、
  ~~(v) `-work` 指標のコストモデルが Agda に無い~~ → **2026-08-09 に Agda 化した**
  （新規 6 モジュール・`--safe`・postulate ゼロ・`check.sh` 108→114 PASS。`RWhileTime` は無改変）。
  **`work(p⁺ on d) = work(p on d) + |⌜p⌝| + 1`** を証明し、**実測 11 行と誤差ゼロで一致**
  （`measured-law` が `refl` で照合）。`-steps` の絶対定数 `+8` と違い work のオーバヘッドは
  プログラム符号の大きさに比例する。**ただし証明したのは基準（p⁺）の側で、残余の work は
  測定値のまま**＝「残余の最適性を証明した」とは書けない。残るのは
  課金③（リテラルパターン）の未モデル化と、抽象層・具象層の橋渡し、
  ~~(vi) `ri_fp3` に `CANON` 修正が入っていない~~ → **2026-08-09 に `CANON-FP3` として移植済み**
  （測定値は 9 行・3 行とも不変。ただしループの脱出条件は両インタプリタとも生の値で比較のまま）、
  (vii) comp2 比の 0.4pt の退行（3 フィールドのフレーム由来）は未調査。
