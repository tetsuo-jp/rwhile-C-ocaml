...
**命令レベルも完了**: `Rwhile.cf` に沿った構文解析器 `pC1`／`pC`／`pThen`/`pElse`/`pDo`/`pLoop`
（先頭トークンごとに 1 節・燃料付きで構造的に停止）に対し、**全 `Cmd` について**

```agda
round-trip : ∀ c → RN c → pC (suc (depthC c)) (tokC c []) ≡ just (c , [])
```

を証明した。`RN c` は「`;` について右結合」（`skip` は分岐位置のみ）という条件で、
**何が印字できるかの制限ではなく、項の括弧の付き方の指定にすぎない** — 上の `seq-assocʳ/ˡ` が
両者の意味論も歩数も同じであることを示しており、印字されるテキストは文字どおり同一だからである。

証明を通すための要点が 2 つあった: (1) 分岐の印字を `if isSkip c then ts else kw ∷ tokC c ts`
