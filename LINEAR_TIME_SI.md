...
`;` ノードも 1 歩数えるので代入 5 個のラッパは 9 歩、前後を繋ぐ `;` を含めて 20 になる。
この数はもはや手計算ではなく**型検査済み**である: `RWhileSIExecTest` の `w-skip`/`w-inv-skip` が
ラッパ付きの実行を `exec` で走らせ、57 = 37 + 20 と 53 = 37 + 16 を `refl` で確認する）:
...
| 対象プログラム | 実装 | モデル | 差 | `skips` |
|---|---:|---:|---:|---:|
| `skip` | 34 | 37 | 3 | **3** |
| `X0 ^= 'a`（定数オペランド） | 178 | 182 | 4 | **4** |
| `X1 ^= X0`（変数オペランド） | 344 | 358 | 14 | — |
| `X0 ^= 'a; X0 ^= 'a` | 516 | 525 | 9 | **9** |
| `from =? X0 nil loop X0 ^= '7 until =? X0 '7` | 1821 | 1869 | 48 | **48** |

**差は一定ではない**（初期の「1 反復あたり 3」という説明は 1 例からの誤った一般化だった。
変数オペランドの 14、ループ例の 48 が反例）。差の源は、本モデルが `skip` を 1 歩と数えるのに対し
R-WHILE の文法では**空の分岐**として印字され実装が課金しないことにある。

これを**モデル内の定理**にした（`RWhileTimeSkip`）:

```agda
skips     : c ⊢ σ ⇒ τ ∣ k → ℕ          -- その実行が実際に踏んだ skip の個数
cost₀     : c ⊢ σ ⇒ τ ∣ k → ℕ          -- skip を 0 歩と数えた勘定
cost-split : ∀ d → k ≡ cost₀ d + skips d
```

そのうえで `skips` を実際に計算すると（`exec-sound` で実行から導出を作り、型検査器が評価）、
**差とちょうど一致する**（3=3、4=4、9=9、48=48。`RWhileSIExecTest` の `n-skip`/`n-ass`/`n-sq`/`n-lp`）。
つまり実装との歩数差は**まるごと空分岐で説明される**。歩行 `walk`
（`from … do skip loop wbody until …`）を含む実行で差が開くのもこれで説明できる。

なお**実装との比較そのものを定理にはできない**: 実装の歩数の数え方は形式化の対象外
（OCaml は検証していない）。上の一致は実測どうしの突き合わせであり、モデル側の
`cost-split` が「差はすべて skip 由来である」ことを保証する、という構図である。
