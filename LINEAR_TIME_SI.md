...
**命令レベル**: `Rwhile.cf` に沿った構文解析器 `pC1`／`pC`／`pThen`/`pElse`/`pDo`/`pLoop`
（先頭トークンごとに 1 節・燃料付きで構造的に停止）を書き、代入・逐次・空 else の条件文・
空 do のループ・入れ子（ループ本体の条件文の中に逐次）・cons 構造をもつ定数の 6 例について
**往復を型検査器が検証**している（`pC 9 (tokC c []) ≡ just (c , [])` を `refl` で）。
全 `Cmd` についての一般定理は次段: 分岐の印字 `tokBr` を
`if isSkip c then ts else kw ∷ tokC c ts` の形に書き直す必要がある（catch-all 節は
`c` の構成子が分からないと簡約しないため）。書き直せば `isSkip` での場合分けで閉じる。
