...
- 歩数は抽出テキストの実測（§4.7）より 1 反復あたり 3 多い: 本モデルは主ループの do 分岐の
  `skip` とループノードに 1 ずつ課金するが、R-WHILE の文法では**空の分岐**として印字され
  コストがかからないため。**モデルの側が保守的**なので、証明した上界は実装にもそのまま効く。
...
必要な箇所だけ `unfolding` する設計変更が要る（見積り: `RWhileSISim` の `one` 呼び出し
34 か所を、`RWhileSIStep` 側で証明した補題に置き換える）。効果は未測定のため、
**計測基盤と原因の特定までを本項の成果とし、リファクタは今後の課題**とする。
