...
-- with the letter's Lemma 1 (`lemma1`) and Theorem 2 (`Main.theorem2`).
-- The chain is:
--
...
init-sound qs b r =
  ...
  , dn vW (λ ()) , dn vT (λ ()) , dn vTmp (λ ()) , g5In , dn vOut (λ ())
  , (λ y → dn (10 + y) (λ ()))
  where
  σ0 : Store
  σ0 = inStore r
  ...
  dn : ∀ y → ¬ (y ≡ vIn) → get ι5 y ≡ nil
  dn y ne = trans (get-set-≢ ι4 vK y (atm qs ∙ atm b) (λ ()))
            (trans (get-set-≢ ι3 vIn y nil (λ ()))
            (trans (get-set-≢ ι2 vR y (encL r) (λ ()))
            (trans (get-set-≢ ι1 vS y (atm b) (λ ()))
            (trans (get-set-≢ σ0 vQ y (atm qs) (λ ()))
                   (z y (λ e → ne (sym e)))))))
  g5L : get ι5 vL ≡ encL []
  g5L = dn vL (λ ())
